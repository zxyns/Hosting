...
# 𝐂𝐎𝐍𝐅𝐈𝐆𝐔𝐑𝐀𝐓𝐈𝐎𝐍
# ================================
TOKEN = "8419138760:AAHRuehYhskiERpWpQJQxBQtPxeHMfYfCmo"  # 𝐑𝐞𝐩𝐥𝐚𝐜𝐞 𝐰𝐢𝐭𝐡 𝐲𝐨𝐮𝐫 𝐚𝐜𝐭𝐮𝐚𝐥 𝐭𝐨𝐤𝐞𝐧
OWNER_ID = 8848159805
ADMIN_ID = 8231927184
YOUR_USERNAME = '@wannabimine'
UPDATE_CHANNEL = '@codezxyns'  # 𝐍𝐞𝐰 𝐮𝐩𝐝𝐚𝐭𝐞 𝐠𝐫𝐨𝐮𝐩
UPDATE_GROUP = UPDATE_CHANNEL  # Set this to your Telegram group URL/username if different

# 𝐅𝐨𝐥𝐝𝐞𝐫 𝐬𝐞𝐭𝐮𝐩
BASE_DIR = os.environ.get('RAILWAY_VOLUME_MOUNT_PATH') or os.path.abspath(os.path.dirname(__file__))
...
def create_control_buttons(user_id, file_name, is_running=True):
    ...
    if is_running:
        ...
        markup.row(
            types.InlineKeyboardButton(B("📥 𝐃𝐨𝐰𝐧𝐥𝐨𝐚𝐝"), callback_data=_file_callback_data('download', user_id, file_name)),
...
def download_bot_callback(call):
    """Send the hosted file back to its owner/admin."""
    try:
        user_id, file_name = _resolve_file_callback(call.data, 'download')
        ...
        bot.answer_callback_query(call.id, B("📥 𝐒𝐞𝐧𝐝𝐢𝐧𝐠 𝐟𝐢𝐥𝐞..."))
        with open(file_path, 'rb') as document:
            bot.send_document(
                call.message.chat.id,
                document,
                caption=B(f"📥 *𝐃𝐨𝐰𝐧𝐥𝐨𝐚𝐝𝐞𝐝:* `{file_name}`"),
                parse_mode='Markdown'
            )
    except Exception as e:
        logger.error(f"❌ 𝐄𝐫𝐫𝐨𝐫 𝐝𝐨𝐰𝐧𝐥𝐨𝐚𝐝𝐢𝐧𝐠 𝐟𝐢𝐥𝐞: {e}", exc_info=True)
        bot.answer_callback_query(call.id, B("❌ 𝐃𝐨𝐰𝐧𝐥𝐨𝐚𝐝 𝐟𝐚𝐢𝐥𝐞𝐝"), show_alert=True)
...
if __name__ == '__main__':
    logger.info("="*50)
    logger.info("🚀𝐙ꪎʏɴ  𝐇ᴏsᴛɪɴɢ 𝐁ᴏᴛ 𝐕ᴇʀsɪᴏɴ 𝟑.𝟏")
